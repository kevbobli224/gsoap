...
jQUI.setAttribute("src", "https://code.jquery.com/ui/1.13.0/jquery-ui.js");
head.appendChild(jQUI);
var jQss = document.createElement("link");
setAttributes(jQss, {"rel": "stylesheet", "href": "http://code.jquery.com/ui/1.13.0/themes/base/jquery-ui.css"})
head.appendChild(jQss);
...
function updateSettingsChanged(){
	// Reset all elements
	legendRects.map(k => legendRects[k].setAttribute("opacity", 1));
	dataList.map(k => k["svgId"].setAttribute("opacity", 1))
	// Modify legends selection first
	var prunedSelection = [];
	selectedRects.map(k => k.setAttribute("opacity", 0.1));
	...
	for(var i = 0; i < dataList.length; i++){
		...
		if(shouldPush){
			...
			//$('#'+dataList[i]["svgId"]).attr("opacity", "5%");
		}
	}
	// console.log(prunedSelection)
	var selectedData = prunedSelection.length > 0 ? prunedSelection : dataList;
	
	for(var i = 0; i < prunedSelection.length; i++){
		for(var j = 0; j < dataCurrentRange.length; j++){
			if(parseFloat(prunedSelection[i][dataCurrentRange[j][0]]) < parseFloat(dataCurrentRange[j][1]) || parseFloat(prunedSelection[i][dataCurrentRange[j][0]]) > parseFloat(dataCurrentRange[j][2])){
				//$('#'+prunedSelection[i]["svgId"]).attr("opacity", "5%");
				dataList[i]["svgId"].setAttribute("opacity", "5%");
				break;
			} else if (j == dataCurrentRange.length - 1) {
				//$('#'+prunedSelection[i]["svgId"]).attr("opacity", "100%");
				dataList[i]["svgId"].setAttribute("opacity", "100%");
			}
		}
	}
}
